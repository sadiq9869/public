...
application = None
data_lock = threading.Lock()  # Added for thread safety
...
def save_users():
    try:
        ...
        with open(USER_FILE + ".backup", "w") as backup:
            json.dump(user_data, backup)
...
def fetch_api_data(url, is_rss=False):
    ...
    try:
        ...
        data = feedparser.parse(url) if is_rss else response.json()
        ...
        # Add cleanup of expired entries
        current_time = time.time()
        API_CACHE = {k: v for k, v in API_CACHE.items() if current_time - v["timestamp"] < 3600}
        return data
            ...
            with data_lock:
                if join_points:
                    ...
                    if top_user_weekly == top_user:
                        ...
                        await context.bot.send_message(GROUP_ID, f"**🏆 {user_profiles.get(top_user, {}).get('handle', top_user)}** ne 1 week top rakha! Key: `{key_name}` 🎉", disable_notification=False)
                    top_user_weekly = top_user
                join_points.clear()
                last_leaderboard_reset = current_time
            await context.bot.send_message(GROUP_ID, "**📊 Leaderboard Reset!** 🚀", disable_notification=False)
...
def process_question(question: str) -> Tuple[str, bool]:
    ...
    try:
        ...
        logger.error(f"Error in generate_inquiry_response: {e}")
        return "Kuch galat ho gaya! Overlord ko bolo! 😡", False
...
async def tutorial(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    tutorial_text = (
        ...
        "**Step-by-Step Guide:**\n"
        "1️⃣ **Permission Lo**\n"
        "   - Trial: /trail (15 attacks, 3 weeks)\n"
        "   - Paid: Overlord se maango!\n\n"
        "2️⃣ **Attack Karo**\n"
        "   - Command: /attack <target> <port> <time>\n"
        "   - Eg: /attack 192.168.1.1 80 120\n"
        "   - **Target:** Valid IP (x.x.x.x)\n"
        "   - **Port:** 1, 2, 6, ya 7 se start\n"
        "   - **Time:** Max 160s (normal), 240s (paid)\n\n"
        "3️⃣ **Rules Samjho**\n"
        "   - Normal: 15 attacks/day, 60s cooldown\n"
        "   - Paid: Unlimited attacks, 30s cooldown\n"
        "   - Profile pic lagao!\n\n"
...
async def myinfo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    info_text = (
        ...
        f"**💡 Change Handle:** /myinfo sethandle <new_handle>"
    )
    ...
    if args and args[0] in ["setnickname", "sethandle"]:
        if len(args) != 2:
            await update.message.reply_text(f"**❌ USAGE:** /myinfo {args[0]} <new_{args[0][3:]}>", parse_mode='Markdown', disable_notification=False)
            return
        new_value = args[1]
    ...
    log_action(user_id, "myinfo", "User checked/updated profile")

async def broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = str(update.effective_user.id)
    if not check_rate_limit(user_id, "broadcast"):
        await update.message.reply_text("**⏳ Thoda Chill Kar!** 5 sec wait kar! 😡", disable_notification=False)
        return
    if user_id not in OVERLORD_IDS:
        await update.message.reply_text("**🚫 OVERLORD ONLY!** 😡", parse_mode='Markdown', disable_notification=False)
        await apply_penalty(user_id, "command_abuse", update, context)
        return
    args = context.args
    if len(args) < 1:
        await update.message.reply_text("**❌ USAGE:** /broadcast <message>", parse_mode='Markdown', disable_notification=False)
        return
    broadcast_msg = " ".join(args)
    await context.bot.send_message(GROUP_ID, f"**📢 OVERLORD BOLTA HAI:** {broadcast_msg} 🚨", parse_mode='Markdown', disable_notification=False)
    update_user_behavior(user_id, "messages_sent")
...
async def mute_unmute(update: Update, context: ContextTypes.DEFAULT_TYPE, is_mute=True):
    user_id = str(update.effective_user.id)
    if not check_rate_limit(user_id, "mute" if is_mute else "unmute"):
        await update.message.reply_text("**⏳ Thoda Chill Kar!** 5 sec wait kar! 😡", disable_notification=False)
        return
    if user_id not in OVERLORD_IDS:
        await update.message.reply_text("**🚫 OVERLORD ONLY!** 😡", parse_mode='Markdown', disable_notification=False)
    ...
    args = context.args
    if len(args) < 1:
        await update.message.reply_text(f"**❌ USAGE:** /{'mute' if is_mute else 'unmute'} <user_id>{' [duration]' if is_mute else ''}", parse_mode='Markdown', disable_notification=False)
        return
    target_id = args[0]
    if not is_mute and target_id not in muted_users:
        await update.message.reply_text("**❌ YEH USER MUTE NHI HAI!** 😡", parse_mode='Markdown', disable_notification=False)
        return
    if is_mute:
        duration = None
        if len(args) > 1:
        ...
        await update.message.reply_text(f"**✅ USER {target_id} MUTED!** Duration: {duration_text} 🔇", parse_mode='Markdown', disable_notification=False)
        await context.bot.send_message(GROUP_ID, f"**🔇 USER {target_id} MUTED!** Duration: {duration_text} 😡", parse_mode='Markdown', disable_notification=False)
    else:
        ...
        await context.bot.send_message(GROUP_ID, f"**🔊 USER {target_id} UNMUTED!** 😊", parse_mode='Markdown', disable_notification=False)
...
async def ban_unban(update: Update, context: ContextTypes.DEFAULT_TYPE, is_ban=True):
    user_id = str(update.effective_user.id)
    if not check_rate_limit(user_id, "ban" if is_ban else "unban"):
        await update.message.reply_text("**⏳ Thoda Chill Kar!** 5 sec wait kar! 😡", disable_notification=False)
        return
    if user_id not in OVERLORD_IDS:
        await update.message.reply_text("**🚫 OVERLORD ONLY!** 😡", parse_mode='Markdown', disable_notification=False)
    ...
    args = context.args
    if len(args) < 1:
        await update.message.reply_text(f"**❌ USAGE:** /{'ban' if is_ban else 'unban'} <user_id>{' [duration]' if is_ban else ''}", parse_mode='Markdown', disable_notification=False)
        return
    target_id = args[0]
    if not is_ban and target_id not in banned_users:
        await update.message.reply_text("**❌ YEH USER BAN NHI HAI!** 😡", parse_mode='Markdown', disable_notification=False)
        return
    if is_ban:
        duration = None
        if len(args) > 1:
        ...
        await update.message.reply_text(f"**✅ USER {target_id} BANNED!** Duration: {duration_text} 🚫", parse_mode='Markdown', disable_notification=False)
        await context.bot.send_message(GROUP_ID, f"**🚫 USER {target_id} BANNED!** Duration: {duration_text} 😡", parse_mode='Markdown', disable_notification=False)
    else:
        ...
        await context.bot.send_message(GROUP_ID, f"**✅ USER {target_id} UNBANNED!** 😊", parse_mode='Markdown', disable_notification=False)
...
async def addall_removeall(update: Update, context: ContextTypes.DEFAULT_TYPE, is_add=True):
    ...
    with data_lock:
        if is_add:
            ...
            await update.message.reply_text("**✅ SABKO ATTACK PERMISSION DI!** 🔓", parse_mode='Markdown', disable_notification=False)
            await context.bot.send_message(GROUP_ID, "**🔓 SABKO ATTACK PERMISSION MIL GAYI!** 💥", parse_mode='Markdown', disable_notification=False)
        else:
            attack_permissions.clear()
            await update.message.reply_text("**✅ SABKI PERMISSION HATA DI!** 🔒", parse_mode='Markdown', disable_notification=False)
            await context.bot.send_message(GROUP_ID, "**🔒 SABKI ATTACK PERMISSION HATA DI!** 🚫", parse_mode='Markdown', disable_notification=False)
...
async def approve_disapprove(update: Update, context: ContextTypes.DEFAULT_TYPE, is_approve=True):
    ...
    args = context.args
    if len(args) != 1:
        await update.message.reply_text(f"**❌ USAGE:** /{'approve' if is_approve else 'disapprove'} <username_or_id>", parse_mode='Markdown', disable_notification=False)
        return
    target_id = args[0]
    with data_lock:
        if is_approve:
            ...
            await update.message.reply_text(f"**✅ USER {target_id} KO PAID FEATURES!** ✅", parse_mode='Markdown', disable_notification=False)
            await context.bot.send_message(GROUP_ID, f"**✅ USER {target_id} PAID MEMBER BAN GAYA!** 🚀", parse_mode='Markdown', disable_notification=False)
        else:
            if target_id in approved_users:
                approved_users.remove(target_id)
            if target_id in paid_users:
                paid_users.remove(target_id)
            await update.message.reply_text(f"**✅ USER {target_id} SE PAID FEATURES HATA DIYE!** 🚫", parse_mode='Markdown', disable_notification=False)
            await context.bot.send_message(GROUP_ID, f"**🚫 USER {target_id} SE PAID FEATURES HATA DIYE!** 😐", parse_mode='Markdown', disable_notification=False)
...
async def gen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if len(args) < 2:
        await update.message.reply_text("**❌ USAGE:** /gen <duration> <device_limit> [key_name]", parse_mode='Markdown', disable_notification=False)
        return
    duration = args[0]
    try:
        device_limit = int(args[1])
    except ValueError:
        await update.message.reply_text("**❌ VALID DEVICE LIMIT DO!** 😡", parse_mode='Markdown', disable_notification=False)
        return
    ...
    with data_lock:
        keys[key_name] = {"duration": duration, "device_limit": device_limit, "devices_used": 0, "expiry_time": expiry_time, "price": price}
    await update.message.reply_text(f"**✅ KEY GENERATED!** Key: `{key_name}`, Duration: {duration}, Limit: {device_limit}, Price: {price} INR 🔑", parse_mode='Markdown', disable_notification=False)
...
async def redeem_access(update: Update, context: ContextTypes.DEFAULT_TYPE, is_code=False):
    ...
    args = context.args
    if len(args) != 1:
        await update.message.reply_text(f"**❌ USAGE:** /{'redeem_code' if is_code else 'redeem'} <{'code' if is_code else 'key_name'}>", parse_mode='Markdown', disable_notification=False)
        return
    key_name = args[0]
    ...
    with data_lock:
        if key_name not in key_dict:
            await update.message.reply_text(f"**❌ INVALID {'CODE' if is_code else 'KEY'}!** 😡", parse_mode='Markdown', disable_notification=False)
            return
        key = key_dict[key_name]
        if key["devices_used"] >= key["device_limit"]:
            await update.message.reply_text(f"**❌ {'CODE' if is_code else 'KEY'} MAX DEVICES PE USED!** 😡", parse_mode='Markdown', disable_notification=False)
            return
        if time.time() > key["expiry_time"]:
            await update.message.reply_text(f"**❌ {'CODE' if is_code else 'KEY'} EXPIRED!** 😡", parse_mode='Markdown', disable_notification=False)
        ...
        if is_code:
            ...
            await update.message.reply_text(f"**✅ CODE REDEEMED!** Trial user bana, {TRIAL_ATTACK_LIMIT} attacks mile! 🕒", parse_mode='Markdown', disable_notification=False)
        else:
            paid_users.add(user_id)
            if user_id in trial_users:
                trial_users.remove(user_id)
                del trial_data[user_id]
            await update.message.reply_text("**✅ KEY REDEEMED!** Paid user bana, unlimited attacks, 30s cooldown! 🚀", parse_mode='Markdown', disable_notification=False)
...
async def add_remove_user(update: Update, context: ContextTypes.DEFAULT_TYPE, is_add=True):
    ...
    args = context.args
    if len(args) != 1:
        await update.message.reply_text(f"**❌ USAGE:** /{'add' if is_add else 'remove'} <user_id>", parse_mode='Markdown', disable_notification=False)
        return
    target_id = args[0]
    with data_lock:
        if is_add:
            attack_permissions.add(target_id)
            await update.message.reply_text(f"**✅ USER {target_id} KO ATTACK PERMISSION!** 🔓", parse_mode='Markdown', disable_notification=False)
        else:
            if target_id in attack_permissions:
                attack_permissions.remove(target_id)
            await update.message.reply_text(f"**✅ USER {target_id} SE PERMISSION HATA DI!** 🔒", parse_mode='Markdown', disable_notification=False)
...
async def attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not check_rate_limit(user_id, "attack"):
        await update.message.reply_text(
            "**⏳ Thoda Chill Kar!** 5 sec wait kar! 😡",
            parse_mode='Markdown',
    ...
    with data_lock:
        ...
        if not photos.photos:
            await update.message.reply_text("**❌ PROFILE PIC LAGAO!** 😡", parse_mode='Markdown', disable_notification=False)
            return
        if user_id not in attack_permissions and user_id not in paid_users and user_id not in trial_users and user_id not in OVERLORD_IDS:
            await update.message.reply_text("**❌ ATTACK PERMISSION NHI HAI!** 😡", parse_mode='Markdown', disable_notification=False)
            return
    ...
    try:
        ...
        await update.message.reply_text(
            "**❌ VALID IP, PORT (1-65535), AUR TIME DO!** 😡",
            parse_mode='Markdown',
            disable_notification=False
        )
        return
    ...
    if int(str(port)[0]) not in VALID_PORT_FIRST_DIGITS:
        await update.message.reply_text(
            "**❌ PORT 1, 2, 6, YA 7 SE START HONA CHAHIYE!** 😡",
            parse_mode='Markdown',
            disable_notification=False
        )
        return
    ...
    with data_lock:
        ...
        max_time = (
            MAX_ATTACK_TIME_OVERLORD if is_overlord else
            MAX_ATTACK_TIME_PAID if is_paid else
            MAX_ATTACK_TIME_NORMAL
        )
        cooldown_time = (
            OVERLORD_COOLDOWN_TIME if is_overlord else
            PAID_COOLDOWN_TIME if is_paid else
            NORMAL_COOLDOWN_TIME
        )
        attack_limit = (
            float('inf') if is_overlord or is_paid else
            TRIAL_ATTACK_LIMIT if is_trial else
            NORMAL_ATTACK_LIMIT
        )
        ...
        if not is_overlord and time.time() - last_attack < cooldown_time:
            ...
            abusive = random.choice(ABUSIVE_WORDS)
            await update.message.reply_text(
                MESSAGE_THEMES["default"]["cooldown"]
                    .replace("[handle]", user_profiles[user_id]["handle"])
        ...
        if not (is_overlord or is_paid):
            ...
            attack_count = attack_counts.get(user_id, {"date": today, "count": 0})
            if attack_count["date"] != today:
                attack_count = {"date": today, "count": 0}
            if attack_count["count"] >= attack_limit:
                await update.message.reply_text(
                    f"**❌ DAILY LIMIT ({attack_limit}) KHATAM!** Kal try kar! 😡",
                    parse_mode='Markdown',
                    disable_notification=False
                )
                return
            if is_trial and trial_data.get(user_id, {}).get("attacks_left", 0) <= 0:
                await update.message.reply_text(
                    "**❌ TRIAL ATTACKS KHATAM!** 😡",
                    parse_mode='Markdown',
                    disable_notification=False
                )
                return

        # Check global attack lock
        if not is_overlord:
            with attack_lock:
                if global_attack_running:
                    await update.message.reply_text(
                        "**❌ EK ATTACK CHAL RAHA HAI!** Wait kar! 😡",
                        parse_mode='Markdown',
                        disable_notification=False
                    )
                    return
        ...
        attack_counts[user_id] = attack_counts.get(user_id, {"date": today, "count": 0})
        attack_counts[user_id]["count"] += 1
        attack_counts[user_id]["date"] = today
        if is_trial:
...
async def attackhistory(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with data_lock:
        attacks = successful_attacks.get(user_id, [])
    if not attacks:
        await update.message.reply_text("**📜 NO ATTACKS!** 😡\nTune abhi tak koi attack nahi kiya.", parse_mode='Markdown', disable_notification=False)
        return
    history = "**📜 ATTACK HISTORY (Last 5) 📜**\n"
    for attack in attacks[-5:]:
        history += f"**🔹** {attack['target']}:{attack['port']}, {attack['time']}s, {attack['timestamp']}\n"
    await update.message.reply_text(history, parse_mode='Markdown', disable_notification=False)
...
async def referral(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with data_lock:
        if not args:
            ...
            await update.message.reply_text(f"**🔗 REFERRAL CODE:** {code}\nDost ko bol: `/referral {code}`", parse_mode='Markdown', disable_notification=False)
        else:
            code = args[0]
            if code not in codes or user_id in codes[code]["used_by"]:
                await update.message.reply_text("**❌ INVALID/USED CODE!** 😡", parse_mode='Markdown', disable_notification=False)
                return
            ...
            await update.message.reply_text("**✅ REFERRAL DONE!** Tu +5, Creator +10 points! 🚀", parse_mode='Markdown', disable_notification=False)
...
async def inquiry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if len(args) < 1:
        await update.message.reply_text("**❌ USAGE:** /inquiry <question>", parse_mode='Markdown', disable_notification=False)
        return
    question = " ".join(args)
    with data_lock:
    ...
    if can_answer:
        ...
        await update.message.reply_text(response_msg, parse_mode='Markdown', disable_notification=False)
    else:
        forward_msg = MESSAGE_THEMES["default"]["inquiry_forward"].replace("[handle]", handle).replace("[question]", question)
        for overlord_id in OVERLORD_IDS:
            await context.bot.send_message(chat_id=overlord_id, text=forward_msg, parse_mode='Markdown', disable_notification=False)
        await update.message.reply_text(response, parse_mode='Markdown', disable_notification=False)
...
if __name__ == '__main__':
    ...
    main()  # Yeh line ensure karegi ki bot shuru ho jaye